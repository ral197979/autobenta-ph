...
function stageColor(key) {
  return STAGES.find(s => s.key === key)?.color || 'bg-surface-container text-on-surface';
}
...
function EditDrawer({ prospect, onClose }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 w-full max-w-sm bg-surface-container-lowest shadow-2xl flex flex-col overflow-y-auto">
      <div className="flex items-center justify-between p-5 border-b border-border-subtle sticky top-0 bg-surface-container-lowest z-10">
        <h2 className="font-bold text-on-surface text-base truncate">{prospect.dealerName}</h2>
        <button onClick={onClose} className="ml-3 shrink-0 text-on-surface-variant hover:text-on-surface">
          <X className="h-5 w-5" />
        </button>
      </div>
      <form onSubmit={handleSubmit} className="p-5 space-y-4 flex-1">
        <div>
          <label className="block text-xs font-semibold text-on-surface-variant mb-1">Closing Stage</label>
        ...
        <div>
          <label className="block text-xs font-semibold text-on-surface-variant mb-1">Next Action</label>
        ...
        <div>
          <label className="block text-xs font-semibold text-on-surface-variant mb-1">Risk Flags (comma-separated)</label>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-surface-container-lowest rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-on-surface">Add to Closing Pipeline</h2>
          ...
          {filtered.map(p => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                selectedId === (p.id || p._id) ? 'bg-blue-50 text-blue-800 font-semibold' : 'hover:bg-surface-container text-on-surface'
              }`}
            >
              {p.dealerName}
              {p.location && <span className="text-on-surface-variant ml-1.5 text-xs">{p.location}</span>}
            </button>
          ))}
          {filtered.length === 0 && <p className="text-sm text-on-surface-variant text-center py-4">No prospects found.</p>}
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-on-surface">Closing Center</h1>
          <p className="text-sm text-on-surface-variant mt-1">Track every deal from demo to payment</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="bg-surface-container-lowest rounded-xl border border-border-subtle p-4 text-center">
      ...
      {isLoading ? (
        ...
        <div className="bg-surface-container-lowest rounded-xl border border-border-subtle p-10 text-center">
          <p className="text-on-surface-variant text-sm">No deals in closing pipeline yet.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {list.map(deal => {
            ...
            return (
              <div key={id} className="bg-surface-container-lowest rounded-xl border border-border-subtle px-4 py-3 flex items-center gap-3 flex-wrap">
                {/* Name + location */}
                <div className="min-w-[140px] flex-1">
                  <p className="font-semibold text-on-surface text-sm leading-tight">{deal.dealerName}</p>
                  {deal.location && <p className="text-xs text-on-surface-variant">{deal.location}</p>}
                </div>
                {/* Stage badge */}
                ...
                {/* Probability */}
                <span className="text-xs text-on-surface font-semibold">{deal.closeProbability ?? 0}%</span>
                {/* Days in stage */}
                <span className="text-[11px] text-on-surface-variant">{days}d in stage</span>
                {/* Risk flags */}
                {risks.map(r => (
                ...
                {/* Next action */}
                {deal.nextAction && (
                  <span className="text-xs text-on-surface-variant italic truncate max-w-[160px]">{deal.nextAction}</span>
                )}
                {/* Owner */}
                {(deal.ownedBy || deal.owner) && (
                  <span className="inline-flex items-center rounded-full bg-surface-container px-2 py-0.5 text-[10px] text-on-surface-variant font-semibold uppercase">
                ...
                <button
                  ...
                  className="ml-auto shrink-0 rounded-lg border border-border-subtle px-3 py-1 text-xs font-semibold text-on-surface hover:bg-surface-container transition-colors"
                >
                  Edit
                </button>
              </div>

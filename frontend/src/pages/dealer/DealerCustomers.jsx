...
function StatCard({ label, value }) {
  return (
    <div className="card p-5 flex flex-col gap-1">
      <span className="text-xs text-slatetext font-medium">{label}</span>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-ink">
          Customers
          <span className="ml-2 text-base font-normal text-slatetext">({customers.length})</span>
        </h1>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1,2,3,4].map(i => (
            <div key={i} className="h-14 bg-softbg rounded-xl animate-pulse" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div className="card p-12 flex flex-col items-center gap-3 text-slatetext">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-cardborder">
              {filtered.map((c, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-softbg/50 transition-colors">
                    <td className="px-5 py-3 font-medium text-ink">{c.buyerName || '—'}</td>
                    <td className="px-5 py-3">
                      <div className="space-y-0.5">
                        {c.buyerEmail && (
                          <a href={`mailto:${c.buyerEmail}`} className="text-electric hover:underline text-xs block truncate max-w-[160px]">
                            {c.buyerEmail}
                          </a>
                        )}
                        {c.buyerPhone && (
                          <a href={`tel:${c.buyerPhone}`} className="text-electric hover:underline text-xs block">
                            {c.buyerPhone}
                          </a>
                        )}
                      </div>
                    ...
                    <td className="px-5 py-3">
                      <div className="text-xs text-slatetext space-y-0.5">
                        {shown.map((v, i) => (
                          <p key={i}>{v.year} {v.make} {v.model}</p>
                        ))}
                        {extra > 0 && (
                          <p className="text-[10px] text-slatetext/70">+{extra} more</p>
                        )}
                        {vehicles.length === 0 && '—'}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center text-ink font-medium">{c.leadCount ?? 0}</td>
                    <td className="px-4 py-3 text-center">
                      {c.wonLeads > 0 ? (
                        ...
                        <span className="text-slatetext">0</span>
                      )}
                    </td>
                    <td className="px-5 py-3 text-xs text-slatetext">
